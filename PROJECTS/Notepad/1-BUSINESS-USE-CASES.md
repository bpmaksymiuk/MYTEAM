USE CASE: UC-001
GOAL
Launch the Chrome plugin and open a new Notepad window that visually resembles the classic Windows Notepad.exe interface (title bar, menu bar with File/Edit/Format/View/Help, text area, status bar).

ACTOR
End User (any Chrome browser user)
- IMPLEMENTATION COMMENT: Chrome popup UI approximates a Notepad window but cannot guarantee desktop-grade free resizing or OS-level chrome parity.

STEP BY STEP WALKTHROUGH
User clicks the Notepad icon in the Chrome toolbar (or uses the keyboard shortcut Ctrl+Shift+N).
...
ACCEPTANCE CRITERIA
Window opens within 500 ms of click.
...
USE CASE: UC-002
GOAL
Create a new blank note/document.

ACTOR
End User
- IMPLEMENTATION COMMENT: Unsaved confirmation is implemented as an extension dialog and not native OS Notepad prompt chrome.

STEP BY STEP WALKTHROUGH
User opens the extension (UC-001) or is already inside an existing note.
...
ACCEPTANCE CRITERIA
Previous content is discarded (with confirmation prompt if unsaved changes exist).
...
USE CASE: UC-003
GOAL
Type, edit, and format basic text inside the note (plain-text editing experience identical to Notepad.exe).

ACTOR
End User
- IMPLEMENTATION COMMENT: Clipboard and redo command support can vary by browser policy; fallback behavior is browser-dependent.

STEP BY STEP WALKTHROUGH
User is inside an open note.
...
ACCEPTANCE CRITERIA
Text behaves exactly like native Notepad (line breaks, tabs, no rich formatting).
...
USE CASE: UC-004
GOAL
Save the current note as a .txt file to the user’s local Downloads folder.

ACTOR
End User
- IMPLEMENTATION COMMENT: Browser controls destination and overwrite UX of downloads; extension cannot force a specific local folder prompt flow.

STEP BY STEP WALKTHROUGH
User clicks File → Save (Ctrl+S) or File → Save As.
...
ACCEPTANCE CRITERIA
File is saved with correct .txt extension and UTF-8 encoding.
...
USE CASE: UC-005
GOAL
Open an existing .txt file from the user’s computer into the Notepad window.

ACTOR
End User
- IMPLEMENTATION COMMENT: ANSI decoding is best-effort through browser text APIs and may not match all legacy code pages exactly.
...
ACCEPTANCE CRITERIA
File content loads correctly (UTF-8 and ANSI supported).
...
USE CASE: UC-006
GOAL
Search and replace text within the current note (Find / Replace functionality).

ACTOR
End User
- IMPLEMENTATION COMMENT: Find/Replace uses a popup dialog that is non-blocking but not independently movable/resizable like a desktop child window.

STEP BY STEP WALKTHROUGH
User clicks Edit → Find (Ctrl+F) or Edit → Replace (Ctrl+H).
...
ACCEPTANCE CRITERIA
Search is case-sensitive when “Match case” is checked.
...
USE CASE: UC-007
GOAL
Automatically save all notes locally (Chrome storage) so they persist across browser restarts and sessions.

ACTOR
End User (background behavior)
- IMPLEMENTATION COMMENT: State persistence is scoped to chrome.storage.local quota and stores the active document snapshot rather than multi-window sessions.

STEP BY STEP WALKTHROUGH
User makes any change in any open note.
...
ACCEPTANCE CRITERIA
All open documents are restored exactly (content + cursor position + title).
...
USE CASE: UC-008
GOAL
Close the Notepad window while optionally saving unsaved changes.

ACTOR
End User
- IMPLEMENTATION COMMENT: Popup closure can be browser-driven; save prompt appears for controlled in-app exit but cannot intercept all external popup closes.

STEP BY STEP WALKTHROUGH
User clicks the close button (X) or File → Exit.
...
ACCEPTANCE CRITERIA
Unsaved changes prompt appears exactly like Notepad.exe.
