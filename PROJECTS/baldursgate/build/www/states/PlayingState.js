...
export class PlayingState {
  ...
  enter() {
    ...
    // HUD button events (btn-inventory, btn-journal, etc.)
  ...
  async _loadArea(areaId) {
    ...
    const COMPANION_ANIM_DEF = {
      frameW: 48, frameH: 64,
      rows: { idle: { baseRow: 0, frameCount: 2 }, walk: { baseRow: 8, frameCount: 4 }, attack: { baseRow: 16, frameCount: 3 } },
    };
    this.npcs = (this.area.npcs || []).map((def, i) => {
      ...
      const spriteKey = `sprite_${def.id}`;
      return {
        ...def,
        id:    def.id || def.name,
        ...
        tileY: def.spawnY ?? 8,
        path: [], velX: 0, velY: 0,
        animDef: { ...COMPANION_ANIM_DEF, sheet: spriteKey },
      };
    }).filter(Boolean);
  ...
  _handleScroll(dt) {
    ...
    // Allow camera to go negative so any tile can be properly centred on screen
  }
