...
After every stage completes, before the next stage begins. The Auditor observes the artifacts produced and checks for governance violations. The Auditor writes findings to `X-AUDIT-REPORT.md` whether violations exist or not — a "Stage N — Clean" entry confirms the audit ran.
...
5. **Traceability coverage** — Do RELATED fields reference valid IDs from the Traceability ID Convention table? Check for orphaned or invented IDs.
...
| Major | Significant quality or process issue; should be resolved before Stage 10. Examples: untraceable IDs, missing RELATED fields. |
