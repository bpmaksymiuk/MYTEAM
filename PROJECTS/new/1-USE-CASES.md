# Approved Use Cases — X-Optimizer / ViralReword

- **Approved:** 2026-05-04
- **APPROVED BY:** Product Owner
- **SOURCE:** `1-USE-CASES-PROPOSED.md` (Stage 0, 2026-05-04)
- **SCOPE NOTE:** All 11 proposed use cases are approved for v1. A/B performance metrics tracking (mentioned in `0-IDEA.md`) requires X API access and is deferred to post-v1; it is not represented in any UC below. Batch-rewrite API endpoint (`POST /batch-rewrite`) is approved as a technical requirement but is not surfaced as a separate user-facing use case at this stage.
...
- **NOTES:** The 3-second latency target applies to standard single-text rewrites. The tool must not alter factual claims present in the original text.
...
- **ACCEPTANCE CRITERIA:**
  ...
  - AC2: At least one strategy must remain active; the system prevents the user from deactivating all strategies and displays an inline warning if they attempt to.
...
- **NOTES:** Hashtag recommendations are based on historical engagement data patterns at v1. Live trending hashtag data via X API is post-v1.
- **RELATED:** UC-001, UC-004.
...
- **ACCEPTANCE CRITERIA:**
  ...
  - AC2: Low-severity issues surface as inline advisory warnings on the tile, not blocking modals.
  - AC3: High-severity issues cause the tile to render a "blocked" state with explanatory text rather than the rewrite text.
  - AC4: Blocked tiles display a brief explanation of why the rewrite was blocked.
...
- **NOTES:** The guardrail evaluates factual accuracy relative to the input only. It does not evaluate political sentiment, opinion, or editorial framing.
...
- **NOTES:** Free tier users have history limited to the last 30 sessions. Pro tier users have unlimited history retained for a minimum of 90 days.
...
- **NOTES:** Rate limits apply per user account, not per IP. Unauthenticated rate limiting is advisory only.
- **RELATED:** UC-010.
...
- [x] All UC records are schema-compliant (GOAL, STEPS, ACCEPTANCE CRITERIA, NOTES, RELATED).
- [x] Approval date header is present: `Approved: 2026-05-04`.
- [x] No UC has vague or unmeasurable acceptance criteria.
- [x] Every UC maps to at least one observable outcome.
- [x] This file is now the single source of approved intent for all downstream stages (2–10).
- [x] `PIPELINE-STATUS.md` is updated for Stage 1 with STATUS and STATUS UPDATED date.
