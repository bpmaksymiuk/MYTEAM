USE CASE:
- USE CASE ID: UC-01
- GOAL: User can install Browser Buddy as a Chrome extension and open the side panel UI.
- ACTOR: End User
- IMPLEMENTATION COMMENT: Requires runtime verification in Chrome to confirm install/open behavior on target environments.
- STEP BY STEP WALKTHROUGH:
  1. User opens Chrome extension settings.
  2. User loads or installs Browser Buddy.
  3. User enables Browser Buddy in the browser.
  4. User clicks the extension action to open the side panel.
- ACCEPTANCE CRITERIA:
  1. Browser Buddy installs without blocking manifest errors.
  2. Browser Buddy appears as enabled in extensions.
  3. User can open the side panel from the action.
...
- STEP BY STEP WALKTHROUGH:
  ...
  5. User reopens the extension UI later.
- ACCEPTANCE CRITERIA:
...
- STEP BY STEP WALKTHROUGH:
  1. User starts capture mode from the extension UI.
  2. User selects a region on the active page.
  3. Extension processes the selected region.
  4. Extension creates and saves a new note from capture output.
- ACCEPTANCE CRITERIA:
  1. Capture mode starts from extension controls.
...
- ACTOR: End User
- STEP BY STEP WALKTHROUGH:
  1. User opens the extension and sees Midnight Terminal applied as the default theme.
  ...
  6. User closes and reopens the extension.
- ACCEPTANCE CRITERIA:
...
USE CASE:
- USE CASE ID: UC-07
- GOAL: User can navigate key extension actions through a clean, tabbed icon-first UI.
- ACTOR: End User
- STEP BY STEP WALKTHROUGH:
...
- ACTOR: End User
- STEP BY STEP WALKTHROUGH:
  1. User opens the NOTES tab in the side panel.
...
- STEP BY STEP WALKTHROUGH:
  1. User opens the FAVORITES tab in the side panel.
...
USE CASE:
- USE CASE ID: UC-11
- GOAL: User can see the extension version number directly in the side panel header next to Browser Buddy.
- ACTOR: End User
- STEP BY STEP WALKTHROUGH:
  1. User opens the Browser Buddy side panel.
  2. User looks at the top branded header.
  3. User sees the product name Browser Buddy.
  4. User sees the current extension version rendered next to Browser Buddy.
- ACCEPTANCE CRITERIA:
  1. Header text includes Browser Buddy followed by a version label.
  2. The displayed version matches the extension manifest version.
  3. Version label is visible in supported side panel widths without clipping.

USE CASE:
- USE CASE ID: UC-12
- GOAL: User can confirm version updates after installing a new release.
- ACTOR: End User
- IMPLEMENTATION COMMENT: Version label refreshes on panel load; users must reopen the side panel after extension update to observe the new version.
- STEP BY STEP WALKTHROUGH:
  1. User notes the current version shown in the side panel header.
  2. User updates Browser Buddy to a newer extension package.
  3. User reopens the side panel.
  4. User compares the header version value to the expected new version.
- ACCEPTANCE CRITERIA:
  1. Version label updates automatically after extension update.
...
- ACTOR: End User
- STEP BY STEP WALKTHROUGH:
  1. User opens the Browser Buddy side panel in default theme.
...
- ACTOR: End User
- STEP BY STEP WALKTHROUGH:
  1. User opens Browser Buddy side panel on the NOTES tab.
...
- ACTOR: End User
- IMPLEMENTATION COMMENT: If the saved favorites subdirectory is deleted outside the extension, startup falls back to root favorites with user-visible notice.
- STEP BY STEP WALKTHROUGH:
